...
- [ ] A reviewer can sign in, configure a Workspace, open a Pull Request, inspect the Review Overview, and enter a Review Queue.
- [ ] A reviewer can mark threads Reviewed, reply, resolve, unresolve, use Bulk Actions, and see correct local/GitHub state separation.
- [ ] A reviewer can inspect file changes, toggle Viewed, use unified/side-by-side diffs, expand context, and fetch full files.
- [ ] A reviewer can resume Review Session state after switching Pull Requests or restarting.
- [ ] Handoff Packets, command palette, Keyboard Flow, privacy controls, diagnostics, and no-telemetry behavior pass acceptance checks.
- [ ] Large Pull Request fixture performance meets the v1 usability threshold.
- [ ] Release pipeline dry-run or real release checks pass for configured platforms.
- [ ] A scope audit confirms out-of-scope items have not been accidentally implemented as unsupported partial features.
